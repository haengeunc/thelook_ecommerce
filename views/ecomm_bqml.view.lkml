    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `@{bigquery_project}.@{bigquery_dataset}.order_items` as oi
                  JOIN `@{bigquery_project}.@{bigquery_dataset}.inventory_items` as inv ON inv.id = oi.inventory_item_id
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `@{bigquery_project}.@{bigquery_dataset}.kmeans_model5`,
        (SELECT * FROM
